#!/usr/bin/env bash
# NCCL (disaggregated) mode: the vLLM server and trainer run on separate GPUs.
# Set INFERENCE_CUDA_DEVICES for inference GPUs and TRAIN_CUDA_DEVICES for training GPUs.
# This script starts the vLLM server automatically before launching training.
#
# Example:
#   INFERENCE_CUDA_DEVICES=2,3 TRAIN_CUDA_DEVICES=0,1 ./countdown_reinforce_nccl.sh
set -euo pipefail
...
INFERENCE_CUDA_DEVICES="${INFERENCE_CUDA_DEVICES:-2,3}"
...
start_vllm_server() {
  echo "Starting vLLM server (NCCL backend) on port ${VLLM_PORT} using GPUs: ${INFERENCE_CUDA_DEVICES}..." >&2
  CUDA_VISIBLE_DEVICES="${INFERENCE_CUDA_DEVICES}" VLLM_SERVER_DEV_MODE=1 \
...
CUDA_VISIBLE_DEVICES="${TRAIN_CUDA_DEVICES}" uv run torchrun \
