    ...
    def train(self):
        ...
        for epoch in range(
            ...
            for tensor_dict in tqdm(
                ...
                self.actor.apo_zero_step(tensor_dict, True, step)
                self.save_ckpt((self.actor,), step)

            for tensor_dict in self.test_dataloader:
                tensor_dict = self.ref_actor.compute_logps(tensor_dict, step, True)
                self.actor.apo_zero_step(tensor_dict, False, step)

        self.save_model((self.actor,))


@hydra.main(config_path="config", config_name="apo_zero", version_base=None)
def main(config: DictConfig):
    init_debugpy_if_enabled()
    initialize_global_process_group()

    trainer = APOZeroTrainer(config)
    trainer.train()
