...
def scatter_data(
    ...
    if num_batches is not None:
        if dist.get_rank() == 0:
            ...
            return [
                scatter_data(
                    None, process_group, multiple_of, max_length_per_dp, pair=pair
                )
...
def gather_data(
    minibatches: List[Dict[str, torch.Tensor]], process_group: dist.ProcessGroup
) -> Optional[Dict[str, torch.Tensor]]:

    minibatches = [
        {k: v.to("cpu") for k, v in minibatch.items()} for minibatch in minibatches
    ]
    minibatches = gather_and_concat_list(minibatches, process_group)
    if dist.get_rank() == 0:
        ...
        for minibatch in minibatches:
            ...
            if pad_tokens > 0:
                ...
                minibatch = {k: v[:, :length] for k, v in minibatch.items()}
            processed_minibatches.append(minibatch)
        tensor_dict = {
            k: torch.cat([minibatch[k] for minibatch in processed_minibatches])
        ...
        tensor_dict = {k: v[reversed_indices] for k, v in tensor_dict.items()}

        if PAD_SEQUENCES > 0:
            tensor_dict = {k: v[:-PAD_SEQUENCES] for k, v in tensor_dict.items()}
...
def count_total(
    ...
    if isinstance(key, tuple):
        return tuple(count_total(minibatches, k, process_group) for k in key)

    total = sum([minibatch[key].sum() for minibatch in minibatches])
    total = torch.Tensor([total]).to(torch.cuda.current_device())
    dist.all_reduce(total, op=dist.ReduceOp.SUM, group=process_group)
...
def slide_along_cp(
    ...
    def _slide_tensor_along_cp(tensor: torch.Tensor) -> torch.Tensor:
        ...
        start_2, end_2 = (2 * cp_size - cp_rank - 1) * chunk_size, (
            2 * cp_size - cp_rank
        ) * chunk_size
    ...
    for k, v in minibatch.items():
        ...
        if length % multiple_of != 0:
            ...
            tensors.append(torch.zeros((pad_tokens), dtype=v.dtype, device=v.device))
        processed_minibatch[k] = torch.cat(tensors).unsqueeze(0)
    seq_lens = torch.LongTensor([len(tensor) for tensor in tensors])
    cu_seqlens = torch.cumsum(
        torch.cat((torch.LongTensor([0]), seq_lens)), dim=0, dtype=torch.int32
...
def gather_along_cp(
    ...
    for k, v in minibatch.items():
        ...
        for start, end in zip(cu_seqlens[:-1], cu_seqlens[1:]):
            tensor = v[start:end]
            tensors = [torch.zeros_like(tensor) for _ in range(cp_size)]
            dist.all_gather(tensors, tensor, group=process_group)
            tensors[cp_rank] = tensor  # necessary to retain grad
            inorder_tensors = [tensor[: len(tensor) // 2] for tensor in tensors]
            reversed_tensors = [tensor[len(tensor) // 2 :] for tensor in tensors[::-1]]
    ...
    if processed_minibatch["eos_mask"][-1].max().item() == 0:
        processed_minibatch = {k: v[:-1] for k, v in processed_minibatch.items()}
