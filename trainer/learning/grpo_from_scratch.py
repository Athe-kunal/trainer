...
# These are scale or shift parameters, not feature mixers
# 1D tensors (dim == 1)
...
for epoch in range(num_epochs):
    ...
    for step, (prompts, answers) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}")):
        ...
        policy_model.train()
        # batch size * K (number of generations per prompt)
        ...
        batch_responses_ids = explore_generations[:, prompt_len:] #(batch_size * K, response_length)
        ...
        # repeat the same advantage for tokens in a completion sequence
        ...
        individual_ppo_reward = torch.min(ratio * batch_advantages, ratio_clipped * batch_advantages)
