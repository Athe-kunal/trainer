...
class FSDPCritic(FSDPWorker):
    ...
    def _forward(
        self, minibatch: Dict[str, torch.Tensor], prefix: str = ""
    ...
    @torch.no_grad()
    def compute_values(
        self, tensor_dict: Optional[Dict[str, torch.Tensor]], step: int
    ...
    def rm_step(
        ...
        for minibatch in progress_bar(minibatches, desc="RM step"):
    ...
    def ppo_update(self, tensor_dict: Optional[Dict[str, torch.Tensor]], step: int):
        ...
        self.model.train()
        tbar = progress_bar(
            total=sum([len(batch) for batch in batches]), desc="Update Critic"
        )
        metrics = defaultdict(list)
