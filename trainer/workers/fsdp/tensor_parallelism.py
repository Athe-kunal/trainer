...
def _to_empty(module: nn.Module):
    # recurse false means only the module itself is moved to the empty device or meta device, not its children
    # moving to meta device means the module is not on any device yet, it has no storage yet, only the shapes are known
    # this is good for memory efficiency, as we don't need to allocate storage for the module and its children
    # later we can materialize the weights onto the device
    module.to_empty(device=torch.cuda.current_device(), recurse=False)


def _sync_module_states(module: nn.Module, attr: str, device_mesh: dist.DeviceMesh):
...
def _prepare_llama_tp_layer(layer: nn.Module, device_mesh: dist.DeviceMesh):
    parallelize_plan = {
        ...
        # It is (batch_size, hidden_dim),
        # This triggers that we should not have an all-reduce after Row Parallel
        # Hence, we have no communication after Row parallel
    ...
    parallelize_module(
        module=layer, device_mesh=device_mesh, parallelize_plan=parallelize_plan
    )
...
def _prepare_llama_tp_actor(model: nn.Module, device_mesh: dist.DeviceMesh):
    ...
    parallelize_plan = {
        "model.embed_tokens": ColwiseParallel(output_layouts=Shard(1)),
    ...
    parallelize_module(
        module=model, device_mesh=device_mesh, parallelize_plan=parallelize_plan
    )
...
def _prepare_llama_tp_critic(model: nn.Module, device_mesh: dist.DeviceMesh):
    ...
    parallelize_plan = {
        ...
        "score": RowwiseParallel(input_layouts=Shard(1)),
    }
...
def prepare_tp_model(model: nn.Module, device_mesh: dist.DeviceMesh) -> None:
    assert (
        model.config.num_key_value_heads % device_mesh.size() == 0
