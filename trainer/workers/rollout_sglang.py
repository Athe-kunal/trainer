...
class Rollout:
    ...
    def __init__(self, config: DictConfig):
        ...
        logger.info(f"Barrier started {get_gloo_group()}")
    ...
    def update(
        ...
        if self.device_mesh["tp"].get_local_rank() == 0:
            sync_request(
                self.worker_url, "resume_memory_occupation", json={"tags": ["weights"]}
            )
        ...
        if self.device_mesh["tp"].get_local_rank() == 0:
            sync_request(
                self.worker_url, "resume_memory_occupation", json={"tags": ["kv_cache"]}
            )
        dist.barrier(group=get_gloo_group())
